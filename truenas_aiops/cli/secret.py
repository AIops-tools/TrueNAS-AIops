"""``truenas-aiops secret`` — manage the encrypted credential store.

Secrets (TrueNAS API keys) are stored in ``~/.truenas-aiops/secrets.enc``
...
from truenas_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
def secret_list() -> None:
    ...
    names = store.names()
    if not names:
        console.print("[yellow]No secrets stored yet. Add one: truenas-aiops secret set <name>[/]")
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update TRUENAS_AIOPS_MASTER_PASSWORD.[/]")
