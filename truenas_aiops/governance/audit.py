"""Unified audit logging engine — all TrueNAS skills write to a single SQLite database.

Replaces 7 per-skill JSON Lines audit loggers with one shared ``~/.truenas-aiops/audit.db``.
...
_log = logging.getLogger("truenas-aiops.audit")

# Back-compat override hook. When None (default) the path resolves via
# ops_path("audit.db") (honoring TRUENAS_AIOPS_HOME). Downstream code/tests may set this
# to a fixed Path to redirect the default DB — kept so the TRUENAS_AIOPS_HOME refactor
