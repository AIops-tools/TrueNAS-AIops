...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``TRUENAS_RUNAWAY_MAX`` times within ``TRUENAS_RUNAWAY_WINDOW_SEC`` trips a
...
from truenas_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing TrueNAS-specific; the decorator wraps it into a
