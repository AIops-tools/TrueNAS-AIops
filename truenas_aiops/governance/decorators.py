"""The ``@governed_tool`` decorator — mandatory wrapper for all TrueNAS MCP tool functions.

Responsibilities:
  1. Pre-check: evaluate policy rules (deny, maintenance window)
  2. Execute: run the actual tool function
  3. Post-log: write audit record to ``~/.truenas-aiops/audit.db``
...
from truenas_aiops.governance.audit import detect_agent, get_engine
from truenas_aiops.governance.budget import BudgetExceeded, get_budget
from truenas_aiops.governance.outcome import (
    clear_prior_state,
    is_unknown,
    take_prior_state,
)
from truenas_aiops.governance.patterns import PatternMatch, get_pattern_engine
from truenas_aiops.governance.policy import PolicyResult, get_policy_engine
from truenas_aiops.governance.readonly import is_read_only, read_only_denial
from truenas_aiops.governance.sanitize import sanitize

_log = logging.getLogger("truenas-aiops.decorators")
...
def governed_tool(
    ...
    *,
    ...
    """Decorator for all TrueNAS MCP tool functions.
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.truenas-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("TRUENAS_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("TRUENAS_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (TRUENAS_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set TRUENAS_AUDIT_APPROVED_BY to the authorizing human (and "
            f"TRUENAS_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _record_undo(state: _CallState, result: Any) -> None:
    ...
    * **Undetermined** — the response was lost (see
      :mod:`truenas_aiops.governance.outcome`). The change may be live, so an
...
def _finalize(state: _CallState) -> None:
    ...
    # cancellation (cancelling mid-flight TrueNAS calls is worse).
...
def _infer_skill(func: Any) -> str:
    ...
    ``truenas_aiops.ops.jobs`` → ``truenas-aiops``
    ``mcp_server.server`` → ``truenas-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("truenas_aiops") or module.startswith("mcp_server"):
        return "truenas-aiops"
    return "unknown"
