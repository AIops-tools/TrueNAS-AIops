"""Encrypted secret storage for truenas-aiops.

Secrets (TrueNAS API keys) are NEVER written to disk in plaintext. They
live in ``~/.truenas-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``TRUENAS_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``TRUENAS_<TARGET>_PASSWORD``; ``prefix='TRUENAS_'`` and
